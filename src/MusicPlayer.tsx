...
export const DynamicIslandMusicPlayer = ({
  ...
  const imageUrl =
    currentSong?.attributes?.artwork?.url
      ?.replace('{w}', '500')
      .replace('{h}', '500') ?? emptyAlbumCover;
  const allImageUrl = song?.map(
    (song) =>
  ...
  return (
    <>
      <Head>
        {allImageUrl?.map((imageUrl: string) => {
          return (
            <>
              <link
                key={imageUrl}
                rel="preload"
                href={`/api/imageProxy?imageUrl=${encodeURIComponent(
                  imageUrl,
                )}`}
                as="image"
              />
      ...
      >
        <MotionDiv
          className="grid justify-center h-full grid-cols-6 ml-1.5"
          size={size}
          before="ultra"
        >
          <MotionDiv
            className="relative col-span-1 mx-auto my-auto overflow-hidden rounded-lg w-7 h-7"
            size={size}
            before="ultra"
          >
          ...
          />
          <MotionDiv
            className="w-7.5 col-span-1 mx-auto my-auto pr-0.5"
            size={size}
            before="ultra"
          >
      ...
      >
        <MotionDiv className="h-full" size={size} before="compact">
          <MotionDiv size={size} before="compact" className="w-full">
            <MotionDiv
              className="grid grid-cols-5 my-6"
              size={size}
              before="compact"
            >
              <MotionDiv
              ...
              >
                <MotionP
                  className="mb-0 font-sans text-sm text-gray-500 truncate"
                  size={size}
                  before="compact"
                >
          ...
          >
            <MotionDiv className="block text-left" size={size} before="compact">
              <MotionP
                className="m-auto font-sans text-sm text-center text-gray-500 align-middle"
                size={size}
                before="compact"
              >
                0:00
